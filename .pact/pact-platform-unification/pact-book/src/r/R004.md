...
Vima 在 profile 中均为可选

## 验收标准（可量化）

禁用 Vima 后 generic 仍通过；另两类返回可解释候选

## 怎么验（`T1`）

| 验收方式 | 判定标准 | 检查者 |
|---|---|---|
| router test 禁用 Vima fixtures | generic 成功，另两类只给候选/诊断 | 脚本 |
...
graph LR
  R003["R003"] --> R004["R004"]
  style R004 fill:#fde68a,stroke:#b45309
```
