# R039 · CLI、Runtime、Skills 与 manifest 同版并支持升…
...
CLI、Runtime、Skills 与 manifest 同版并支持升级后再同步

## 验收标准（可量化）

package version 与 manifest version 相等；版本/hash 漂移由 doctor 检出，sync 只采用当前包内容

## 怎么验（`T1`）

| 验收方式 | 判定标准 | 检查者 |
|---|---|---|
| package/manifest/drift fixtures | 同版同 hash PASS；版本或内容漂移 doctor 非零且不写目标 | 脚本 |

## 依赖关系

- **依赖**（要先做完这些）：[R030](../r/R030.md)、[R035](../r/R035.md)
- **被依赖**（这条不稳，下面这些都会塌）：[R040](../r/R040.md)、[R042](../r/R042.md)
...
graph LR
  ...
  R039 --> R040["R040"]
  R039 --> R042["R042"]
  style R039 fill:#fde68a,stroke:#b45309
```
