 ...
 * 宁可略宽也不能算窄——算窄会直接挤掉按钮。
 */
export function buttonLabelWidth(label: string) {
  return [...label].reduce((width, char) => width + (FULL_WIDTH_CHAR.test(char) ? 14 : 8), 0);
}

/** 单个按钮的占位宽度 */
export function buttonWidth(label: string) {
  return Math.max(BUTTON_MIN_WIDTH, Math.ceil(buttonLabelWidth(label.trim())) + BUTTON_FRAME_WIDTH);
}

/** 一行操作单元格里全部按钮所需的宽度；该行没有按钮时返回 0（不参与取最大值） */
export function operationCellWidth(labels: string[]) {
  const rendered = labels.filter((label) => label.trim());
  if (!rendered.length) return 0;
  return rendered.reduce((width, label) => width + buttonWidth(label) + BUTTON_GAP, 0) + CELL_PADDING_X;
}
 ...
 * 只看一行会让另一些行的按钮被挤掉。
 */
export function operationColumnWidth(rowLabels: string[][]) {
  return Math.max(OPERATION_COLUMN_MIN_WIDTH, ...rowLabels.map(operationCellWidth));
}
...
export const FLEXIBLE_COLUMN_MIN_WIDTH = 160;
 ...
 * 而不是把某一列压塌。
 */
export function tableMinWidth(columnWidths: number[], hasCheckColumn: boolean) {
  const total =
    columnWidths.reduce((sum, width) => sum + width, 0) + (hasCheckColumn ? CHECK_COLUMN_WIDTH : 0);
  return Math.max(TABLE_MIN_WIDTH, total);
}
